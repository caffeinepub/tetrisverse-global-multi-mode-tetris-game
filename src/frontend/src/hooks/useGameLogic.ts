...
function tryRotate(
  ...
  for (const [dx, dy] of kicks) {
    const kicked = {
      ...rotated,
      position: { x: rotated.position.x + dx, y: rotated.position.y + dy },
    };
...
export function useGameLogic(): GameLogicReturn {
  ...
  const spawnPiece = useCallback(
    (
      ...
      mode: GameMode,
    ): { piece: Tetromino; nextPieces: Tetromino[] } => {
      const _config = MODE_CONFIGS[mode];
  ...
  const lockPiece = useCallback(
    (state: GameState, mode: GameMode): GameState => {
      ...
      const { piece: nextPiece, nextPieces: newNextPieces } = spawnPiece(
        clearedBoard,
        state.nextPieces,
        mode,
      );
  ...
  const tick = useCallback(() => {
    setGameState((prev) => {
      ...
      const config = MODE_CONFIGS[mode];
      const direction = config.gravityShiftMode
        ? prev.gravityDirection
        : "down";
  ...
  const getSpeed = useCallback((level: number, mode: GameMode): number => {
    ...
    if (mode === "insanity") {
      return Math.max(
        50,
        config.initialSpeed - level * config.speedIncrement * 2,
      );
    }
  ...
  const startGame = useCallback(
    (mode: GameMode) => {
      ...
      const isInsanity = mode === "insanity";
      const initialBoard = config.puzzleMode
        ? createPuzzleBoard()
        : createEmptyBoard();
  ...
  useEffect(() => {
    ...
    if (intervalRef.current) {
      ...
      intervalRef.current = setInterval(
        tick,
        getSpeed(gameState.level, currentMode),
      );
    }
  }, [
    gameState.level,
