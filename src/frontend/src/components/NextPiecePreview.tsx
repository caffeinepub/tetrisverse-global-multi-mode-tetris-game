...
import { TETROMINO_COLORS, type Tetromino } from "../types/game";
...
export default function NextPiecePreview({
  ...
  const PREVIEW_CELL = horizontal
    ? PREVIEW_CELL_HORIZONTAL
    : PREVIEW_CELL_VERTICAL;
  ...
  useEffect(() => {
    ...
    pieces.slice(0, count).forEach((piece, idx) => {
      ...
      const startX =
        offsetX + Math.floor((previewW - pieceW) / 2) - minX * PREVIEW_CELL;
      const startY =
        offsetY + Math.floor((previewH - pieceH) / 2) - minY * PREVIEW_CELL;
