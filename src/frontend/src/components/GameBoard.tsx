...
function useViewportCellSize() {
  ...
  useEffect(() => {
    function calculate() {
      ...
      const size = Math.max(
        14,
        Math.min(byHeight, byWidth, isMobile ? 28 : 34),
      );
      setCellSize(size);
    }
...
export default function GameBoard({
  ...
  const stats = [
    ...
    ...(gameState.combo > 1
      ? [{ label: t.combo, value: `x${gameState.combo}` }]
      : []),
    ...(gameState.timeLeft !== undefined
      ? [{ label: t.time, value: formatTime(gameState.timeLeft) }]
      : []),
    ...(gameState.movesLeft !== undefined
      ? [{ label: t.moves, value: gameState.movesLeft }]
      : []),
  ];
    ...
    >
      ...
      >
        ...
        <div className="text-center">
          ...
          <div className="text-xs opacity-60" style={{ fontSize: "10px" }}>
            {config.difficulty}
          </div>
        </div>
        <button
      ...
      >
        ...
        <div className="flex items-center justify-center gap-2 w-full mb-1">
          ...
          >
            <div
              ...
              style={{ fontSize: "9px", opacity: 0.6, color: text }}
            >
              NEXT
            </div>
            <NextPiecePreview
        ...
        <div
          className="flex gap-1 w-full justify-center mb-1"
          style={{ flexWrap: "wrap" }}
        >
