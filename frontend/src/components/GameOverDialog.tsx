...
import { GameMode, MODE_CONFIGS } from '../types/game';
...
export default function GameOverDialog({
  score, lines, level, won, mode, theme, onRestart, onMenu, onLeaderboard
}: GameOverDialogProps) {
  const { t } = useLanguage();
      ...
      >
        ...
        <div className="text-center mb-6">
          {won ? (
            <div className="flex flex-col items-center gap-2">
              ...
              <h2 className="text-3xl font-bold" style={{ color: '#ffd700' }}>{t.youWin}</h2>
            </div>
          ) : (
            <h2 className="text-3xl font-bold" style={{ color: primary }}>{t.gameOver}</h2>
          )}
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          {[
            ...
            <div
              ...
              style={{ background: isLight ? 'rgba(0,0,0,0.05)' : 'rgba(255,255,255,0.05)', border: `1px solid ${border}` }}
            >
              <div className="text-xl font-bold" style={{ color: primary }}>{value}</div>
        ...
        {!submitted ? (
          <div className="mb-4">
            ...
            <button
              ...
              style={{ background: primary, color: theme.isDark ? '#000' : '#fff' }}
            >
              {submitScore.isPending ? '...' : t.submitScore}
            </button>
          </div>
        ) : (
          <div className="text-center text-sm mb-4 opacity-70">✓ Score submitted!</div>
        )}
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="flex-1 flex items-center justify-center gap-1 rounded-lg py-2 text-sm font-semibold"
            style={{ background: isLight ? 'rgba(0,0,0,0.08)' : 'rgba(255,255,255,0.1)', border: `1px solid ${border}`, color: text }}
          >
