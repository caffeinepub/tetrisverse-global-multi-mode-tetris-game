...
export default function MissionNotification() {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('missionComplete', handleMissionComplete as EventListener);
  }, []);
    ...
    <div className="fixed top-16 right-4 z-50 flex flex-col gap-2 pointer-events-none">
      {notifications.map(n => (
        <div
          ...
          style={{
            background: theme.isDark ? 'rgba(10,10,30,0.97)' : 'rgba(255,255,255,0.97)',
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1 mb-0.5">
              ...
              <span className="text-xs font-bold uppercase tracking-wide" style={{ color: theme.accentColor }}>
                Mission Complete!
              </span>
            </div>
            <p className="text-sm font-semibold truncate" style={{ color: theme.textColor }}>{n.title}</p>
            <p className="text-xs opacity-60" style={{ color: theme.textColor }}>Reward: {n.reward}</p>
          </div>
          <button
