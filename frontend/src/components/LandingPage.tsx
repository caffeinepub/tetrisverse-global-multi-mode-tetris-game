import { ExternalLink, FileText, CheckCircle, Shield } from 'lucide-react';

export default function LandingPage() {
  const currentYear = new Date().getFullYear();
  const appId = typeof window !== 'undefined' ? window.location.hostname : 'tetrisverse-app';
        ...
        <div className="max-w-2xl w-full mx-auto text-center space-y-10">
          ...
          <div className="bg-game-card border border-game-border rounded-2xl p-6 text-left space-y-4">
            ...
            <p className="text-game-text text-base leading-relaxed">
              This is the official developer website for <strong className="text-neon-cyan">TetrisVerse</strong> — 
              a global multi-mode Tetris game available on Android. This site hosts the{' '}
              <code className="bg-game-code text-neon-green px-1.5 py-0.5 rounded text-sm font-mono">app-ads.txt</code>{' '}
            ...
            <div className="flex items-start gap-2 text-game-muted text-sm">
              ...
              <span>Publisher ID: <span className="text-game-text font-mono">pub-7936595519986908</span></span>
            </div>
          </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div className="text-left">
                  <p className="font-bold text-game-text text-base">app-ads.txt</p>
                  <p className="text-game-muted text-sm font-mono">/app-ads.txt</p>
                </div>
              </div>
          ...
          <p className="text-game-muted text-sm leading-relaxed">
            Google AdMob uses this file to verify that TetrisVerse is an authorized publisher. 
            The file is accessible at{' '}
            <code className="text-neon-cyan font-mono text-xs bg-game-code px-1.5 py-0.5 rounded">
              {typeof window !== 'undefined' ? window.location.origin : 'https://[domain]'}/app-ads.txt
            </code>
          </p>
        </div>
      </main>
