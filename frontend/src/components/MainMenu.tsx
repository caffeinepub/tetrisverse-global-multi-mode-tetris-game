...
import { Play, Trophy, Target, Settings } from 'lucide-react';
...
export default function MainMenu({ onPlay, onLeaderboard, onMissions, onSettings }: MainMenuProps) {
  const { theme } = useGameTheme();
  const { t } = useLanguage();

  const menuButtons = [
    { label: t.play, icon: <Play size={20} />, onClick: onPlay, primary: true },
    { label: t.leaderboard, icon: <Trophy size={20} />, onClick: onLeaderboard, primary: false },
    { label: t.missions, icon: <Target size={20} />, onClick: onMissions, primary: false },
    { label: t.settings, icon: <Settings size={20} />, onClick: onSettings, primary: false },
  ];
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: theme.isDark ? 'rgba(0,0,0,0.65)' : 'rgba(255,255,255,0.6)' }}
      />
      ...
      <div className="relative z-10 flex flex-col items-center gap-8 px-4 py-8">
        ...
        <div className="flex flex-col items-center gap-4">
          ...
          <p
            ...
            style={{ color: theme.isDark ? 'rgba(255,255,255,0.7)' : 'rgba(0,0,0,0.6)' }}
          >
        ...
        <p
          className="text-xs"
          style={{ color: theme.isDark ? 'rgba(255,255,255,0.3)' : 'rgba(0,0,0,0.3)' }}
        >
          v1.0.0
        </p>
      </div>
