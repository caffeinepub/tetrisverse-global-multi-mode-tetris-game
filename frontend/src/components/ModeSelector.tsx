...
const MODE_ICONS: Partial<Record<GameMode, string>> = {
  ...
  gravityShift: '/assets/generated/gravity-shift-mode-icon-transparent.dim_64x64.png',
...
export default function ModeSelector({ onSelect, onClose }: ModeSelectorProps) {
  ...
  const modes = Object.entries(MODE_CONFIGS) as [GameMode, typeof MODE_CONFIGS[GameMode]][];
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50" style={{ background: 'rgba(0,0,0,0.75)' }}>
      ...
      >
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-2xl font-bold" style={{ color: primary }}>{t.selectMode}</h2>
          <button
            onClick={onClose}
          ...
          {modes.map(([modeKey, config]) => (
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                {MODE_ICONS[modeKey] ? (
                  <img src={MODE_ICONS[modeKey]} alt={config.name} className="w-8 h-8 object-contain" />
                ) : (
              ...
              <p className="text-xs opacity-60 leading-relaxed">{config.description}</p>
