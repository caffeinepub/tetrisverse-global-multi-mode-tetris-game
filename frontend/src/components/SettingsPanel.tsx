...
import { useGameTheme, THEMES } from '../contexts/GameThemeContext';
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col h-full max-w-2xl mx-auto w-full px-4 overflow-auto">
        {/* Header */}
        <div className="flex items-center gap-3 py-4 border-b flex-shrink-0" style={{ borderColor: theme.borderColor }}>
          <button
            onClick={onBack}
          ...
          <h2 className="text-xl font-bold" style={{ color: theme.textColor }}>{t.settings}</h2>
        </div>

        <div className="flex flex-col gap-6 py-4">
          {/* Audio section */}
          <section>
            <h3 className="text-sm font-semibold mb-3 uppercase tracking-wider" style={{ color: theme.accentColor }}>
              {t.audio}
            </h3>
            <div
              className="flex items-center justify-between p-4 rounded-lg"
              style={{
                background: theme.isDark ? 'rgba(0,0,0,0.6)' : 'rgba(255,255,255,0.85)',
                border: `1px solid ${theme.borderColor}`,
              }}
            >
              <div className="flex items-center gap-3">
                {isMuted ? (
                  <VolumeX size={20} style={{ color: theme.isDark ? 'rgba(255,255,255,0.4)' : 'rgba(0,0,0,0.4)' }} />
                ) : (
              ...
              {allThemes.map(t2 => (
                <button
                  ...
                  style={{
                    ...
                    boxShadow: themeName === t2.id ? `0 0 12px ${t2.glowColor}66` : 'none',
                    aspectRatio: '4/3',
                  }}
                >
                  {t2.backgroundImage && (
                  ...
                  >
                    <span className="text-white text-xs font-semibold drop-shadow">{t2.displayName}</span>
                  </div>
                  {themeName === t2.id && (
                    <div
              ...
              {allLanguages.map(lang => (
                <button
                  ...
                  style={language === lang ? {
                    ...
                    background: theme.isDark ? 'rgba(0,0,0,0.5)' : 'rgba(255,255,255,0.8)',
          ...
          <section>
            ...
            >
              <p className="font-semibold mb-1" style={{ color: theme.textColor }}>TetrisVerse v1.0.0</p>
              <p className="text-xs mb-2">The ultimate multi-mode Tetris experience with 9 game modes and 8 themes.</p>
              <a
                href="/app-ads.txt"
                target="_blank"
